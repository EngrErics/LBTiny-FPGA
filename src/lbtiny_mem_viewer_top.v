...
module lbtiny_mem_viewer_top (
    ...
    // Sticky request flags. Each flag sets on the debounced rising edge of its
    // button (cross-masked so a held BTNU ignores BTNC and vice versa) and
    // stays set until the controller FSM clears it when it acts on the press.
    // This guarantees the FSM never misses a press regardless of where it is
    // in its cycle when the button fires.
    reg btnc_req = 1'b0;
    reg btnu_req = 1'b0;
    ...
    always @(posedge clk_bus) begin
        if (btnc_edge) btnc_req <= 1'b1;
        else if (btnc_req && state == S_IDLE) btnc_req <= 1'b0;

        if (btnu_edge) btnu_req <= 1'b1;
    ...
    always @(posedge clk_bus) begin
        ...
        case (state)
            ...
            S_IDLE: begin
                ...
                fill_busy   <= 1'b0;
                if (btnc_req) begin
                    // btnc_req cleared by the always block above this cycle.
                    viewer_busy <= 1'b1;
                    fill_busy   <= 1'b1;
                    fill_addr   <= 12'h000;
                    step        <= 3'd0;
                    ...
                    // btnu_req cleared by the always block above this cycle.
                    viewer_busy <= 1'b1;
                    step        <= 3'd0;
                    state       <= S_INIT_ERASE_NEXT;
                end else begin
